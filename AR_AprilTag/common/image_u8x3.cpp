...
image_u8x3_t *image_u8x3_create_alignment(unsigned int width, unsigned int height, unsigned int alignment)
{
	...
	// const initializer
	image_u8x3_t *tmp = new image_u8x3_t(width, height, stride, buf);
...
image_u8x3_t *image_u8x3_copy(const image_u8x3_t *in)
{
	...
	// const initializer
	image_u8x3_t *tmp =new image_u8x3_t(in->width,in->height,in->stride,buf );
...
void image_u8x3_gaussian_blur(image_u8x3_t *im, double sigma, int ksz)
{
	...
	for (int c = 0; c < 3; c++) {
		for (int y = 0; y < im->height; y++) {
			...
			uint8_t *in = new uint8_t[im->stride];
			uint8_t *out = new uint8_t[im->stride];
		...
		for (int x = 0; x < im->width; x++) {
			uint8_t *in = new uint8_t[im->height];
			uint8_t *out = new uint8_t[im->height];
