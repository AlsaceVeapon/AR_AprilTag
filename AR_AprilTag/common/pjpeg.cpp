...
extern void pjpeg_idct_2D_double(int32_t in[64], uint8_t *out, uint32_t outstride);
extern void pjpeg_idct_2D_u32(int32_t in[64], uint8_t *out, uint32_t outstride);
extern void pjpeg_idct_2D_nanojpeg(int32_t in[64], uint8_t *out, uint32_t outstride);
...
static int pjpeg_decode_buffer(struct pjpeg_decode_state *pjd)
{
	...
	while (bd_has_more(&bd)) {
		...
		case 0xda: { // Start Of Scan (SOS)
			...
			uint8_t *comp_idx = new uint8_t[ns];
